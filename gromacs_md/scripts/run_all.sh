...
# ---------- 模拟参数配置 ----------
TESTING="${TESTING:-0}"
if [ "${TESTING}" = "1" ] || [ "${2:-}" = "--testing" ] || [ "${2:-}" = "-Testing" ] || [ "${SIM_MODE:-}" = "test" ]; then
    MDP="../mdp/test"
    TESTING="1"
    echo ">> [5,000 步从零贯通验证模式] 使用 mdp/test (不省略任何前置步骤: 建系 -> EM -> NVT -> NPT -> 5000步MD)"
...
# ----- 从头全流程运行模式: 自动清理上一次历史残留断点与文件, 确保 100% 从零算起 -----
echo ">> [从头全流程模式] 正在自动清理体系 ${SYS} 历史断点文件 (*.cpt, *.tpr, *.xtc ...)，保证 100% 从头计算 ..."
rm -f *.cpt *.tpr *.edr *.xtc *.log *.trr *.gro *.ndx topol.top posre*.itp 2>/dev/null || true
